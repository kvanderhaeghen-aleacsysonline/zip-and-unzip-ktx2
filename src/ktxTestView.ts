...
import { getAnimationAssetPaths, getAssetsKTXTestPaths, getSpineAssetPath } from './constants/constants';
...
export class KTXTestView {
    ...
    public async createTestSpine(spriteCount: number, type?: KTX2Types): Promise<void> {
        return new Promise<void>(async (resolve) => {
            const spritePaths = getSpineAssetPath(type);
            const atlas = (await (await fetch(spritePaths.path + '/' + spritePaths.atlas)).text()) ?? '';
            const animation = (await (await fetch(spritePaths.path + '/' + spritePaths.anim)).text()) ?? '';
            console.error(spritePaths, atlas, animation, type);

            try {
                const textureAtlas: PixiSpine.TextureAtlas = new PixiSpine.TextureAtlas(atlas);
                const skeletonParser = new PixiSpine.SkeletonJson(new PixiSpine.AtlasAttachmentLoader(textureAtlas));
                ...
                if (spineData) {
                    for (let i = 0; i < spriteCount; i++) {
                        ...
                        baseSpine.position.set(
                            _.random(baseSpine.width * 0.05, this.canvasApp.screen.width - baseSpine.width * 0.05),
                            _.random(baseSpine.height * 0.05, this.canvasApp.screen.height - baseSpine.height * 0.05)
                        );
    ...
    public dispose(): void {
        ...
        for (let i = 0; i < this.testObjects.spines.length; i++) {
        ...
        this.testObjects.spines.splice(0);
