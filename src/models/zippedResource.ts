...
export class ZippedResource {
    ...
    public getPixiTexture(path: string): Pixi.Texture | undefined {
        const pathArray = path.split('/');
        const fileName = pathArray[pathArray.length - 1];

        const isKtx2 = fileName.includes('ktx2');
        if (isKtx2) {
            const texture = this.getKTX2Texture(fileName);
            return texture;
        }

        const texture = this.getNormalTexture(fileName);
        return texture;
    }

    public getNormalTexture(fileName: string): Pixi.Texture | undefined {
    ...
    public getKTX2Texture(fileName: string): Pixi.Texture | undefined {
        const byteArr = this.unzipResource(fileName);
        if (!byteArr) return undefined;
