...
export function getTextureAssetPaths(ktx2Type?: KTX2Types): string[] {
  if (ktx2Type) {
    ...
    return assetTexturePaths.map((item) =>
      item.replace("./assets", texturePath).replace(/jpg|jpeg|png/g, "ktx2"),
    );
  }
  return assetTexturePaths;
}
export function getAnimationAssetPaths(
  isPOTS: boolean,
  ktx2Type?: KTX2Types,
): string[] {
  ...
  const paths = _.times(
    data.length,
    (index) => `${data.path}${index + 1}.${data.ext}`,
  );
  if (ktx2Type) {
    const texturePath = getKTX2TypePath(ktx2Type);
