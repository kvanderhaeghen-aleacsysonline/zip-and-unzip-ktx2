...
export class WebGLProfiler {
  ...
  constructor(context: WebGLRenderingContext | WebGL2RenderingContext) {
    ...
    this.ext = context.getExtension(
      "EXT_disjoint_timer_query",
    ) as EXTDisjointTimerQuery | null;
  }
  ...
  start(): void {
    ...
    if (infoExt != null) {
      const renderer: string = this.context.getParameter(
      ...
      if (renderer.indexOf("NVIDIA GeForce GT 750M") !== -1) {
        ...
        throw new Error(
          `${renderer} cards seem to have a buggy implementation of EXT_disjoint_timer_query. Refusing to record to avoid misleading results.`,
        );
      }
    }
  ...
  popContext(name: string): void {
    ...
    if (popped !== name) {
      throw new Error(
        `Expected popContext to be called with ${popped}, but it was called with ${name}`,
      );
    }
  ...
  private resolveEventsIfPossible(): void {
    ...
    while (i < this.eventsPendingTimestamps.length) {
      const pendingAction = this.eventsPendingTimestamps[i];
      ...
      const elapsed = this.ext.getQueryObjectEXT(
        query,
      ...
      const timestamp = lastTimestamp + elapsed;
  ...
  private toSpeedscopeProfile(): string {
    ...
    const speedscopeEvents: (
      | SpeedscopeOpenFrameEvent
      | SpeedscopeCloseFrameEvent
    )[] = [];
    ...
    for (const event of this.resolvedEvents) {
      speedscopeEvents.push({
        type:
          event.action.type == GPUProfilerActionType.OPEN_FRAME
...
interface EXTDisjointTimerQuery {
  ...
  getQueryObjectEXT(
    ...
    pname: 0x8867 /* QUERY_RESULT_AVAILBLE_EXT */,
  ): boolean;
  getQueryObjectEXT(
    query: WebGLTimerQueryEXT,
    pname: 0x8866 /* QUERY_RESULT_EXT */,
  ): number;
