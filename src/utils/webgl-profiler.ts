...
export class WebGLProfiler {
    ...
    private resolveEventsIfPossible(): void {
        ...
        while (i < this.eventsPendingTimestamps.length) {
            ...
            const lastTimestamp = this.resolvedEvents.length === 0 ? 0 : this.resolvedEvents[this.resolvedEvents.length - 1].timestamp;
...
type SpeedscopeValueUnit = 'none' | 'nanoseconds' | 'microseconds' | 'milliseconds' | 'seconds' | 'bytes';
