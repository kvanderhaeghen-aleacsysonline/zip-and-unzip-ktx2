...
export class Project implements IProject {
    ...
    private calculateScaleRatio(): number {
        ...
        return window.screen.orientation.type.includes('landscape') ? scaleX : scaleY;
    }
    ...
    private async createButtons(): Promise<void> {
        ...
        this.createButton('Sound', this.loadContainer, 20 + scaleW + offset, offset, scaleW * 0.45, scaleH, 24 * screenScale, async () => {
        ...
        this.createButton('Load local zip', this.soundContainer, 20, offset + height * 3, scaleW, scaleH, 24 * screenScale, async () => {
            if (this.isLoading) return;
            this.isLoading = true;
            this.isSpritesTest = false;
            this.disposeAll();
    ...
    private makeDraggableSprite(sprite: Pixi.Sprite): void {
        ...
        sprite
            .on('pointerdown', this.onDragStart, sprite)
