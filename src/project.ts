...
export class Project implements IProject {
    ...
    public async launch(): Promise<void> {
        ...
        window.addEventListener('resize', () => {
            this.canvasApp.renderer.resize(window.innerWidth - 20, window.innerHeight - 20);
        });
    ...
    private createButtons(): void {
        ...
        this.resultText = this.createResultText('Logs:', 20 * screenScale, 20, offset + height * 6).text;
        void this.printRenderer();

        this.createButton('Save', this.saveContainer, 20, offset, scaleW, scaleH, 24 * screenScale, async () => {
        ...
        this.createButton('Sound', this.loadContainer, 20 + scaleW + offset, offset, scaleW * 0.45, scaleH, 24 * screenScale, () => {
        ...
        const ktx2Btn = this.createButton(
            ...
            async () => {
                ...
                ktx2Btn.text.text = this.ktx2Type ? 'KTX2 ' + this.ktx2Type?.toUpperCase() : 'KTX2 Disabled';
        ...
        this.createButton('Load local zip', this.soundContainer, 20, offset + height * 3, scaleW, scaleH, 24 * screenScale, () => {
            if (this.isLoading) return;
            this.isLoading = true;
            this.isSpritesTest = false;
            this.disposeAll();
        ...
        this.createButton(
            ...
            async () => {
                ...
                const data = await fetch(url + `?${timestamp}`).then((res) => res.arrayBuffer());
                this.logResults('Downloaded zip in ', Date.now() - time1, 'ms');
                const time2 = Date.now();
                const imageExt = this.ktx2Type ? 'KTX2_' + this.ktx2Type.toUpperCase() : 'PNG/JPEG';
        ...
        this.createButton('Quality\nSprite', this.loadContainer, 20, offset + height * 4.5, scaleW * 0.45, scaleH, 18 * screenScale, async () => {
            this.isSpritesTest = false;
            this.disposeAll();
        ...
        this.createButton(
            ...
            async () => {
                ...
                await this.ktxTestViewer.createTestSprites(1000, this.isPOTS, this.ktx2Type);
        ...
        this.createButton(
            ...
            async () => {
                ...
                await this.ktxTestViewer.createTestAnimation(1000, this.isPOTS, this.ktx2Type, 0.2);
    ...
    private createResultText(name: string, fontSize: number, x: number, y: number): { text: Pixi.Text } {
        const text = new Pixi.Text({
            text: name,
            style: {
                fontFamily: 'Arial',
    ...
    public async createResources(): Promise<void> {
        ...
        for (let i = 0; i < length; i++) {
            ...
            try {
                const assetLoad = texturePath.includes('.ktx2') ? { src: texturePath, loadParser: 'loadKTX2' } : { src: texturePath };
